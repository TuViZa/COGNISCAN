...
async def upload_video(file: UploadFile = File(...)):
    try:
        ...
        # Process the video (mock for now)
...
async def process_video(file_path: str) -> Dict:
    ...
    print(f"Processing complete. Score: {score:.1f}")
...
async def get_patient_history(patient_id: str):
    ...
    """
    print(f"Getting history for patient: {patient_id}")
...
@app.get("/")
async def root():
    return {"message": "Cogniscan API is running", "status": "healthy"}

if __name__ == "__main__":
    print("Starting Cogniscan Backend...")
